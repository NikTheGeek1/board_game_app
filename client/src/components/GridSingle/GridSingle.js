...
const Grid = ({ onSetUserScores, resetState, setResetState, setPlayerStats }) => {
    ...
    /*
        This useEffect runs only if the machine is playing 
    */
    useEffect(() => {
        const tmInterval = setTimeout(() => {


            if (currentPlayer == 'machine') {
                const moveObj = gridInstance.movePieceMachine();
                ...
                if (gridInstance.captures.user1.score === 12 || gridInstance.captures.user2.score === 12) {
                    ...
                    const winnerPlayer = currentPlayer;
                    setCurrentPlayer('');
                    return setWinner(usersObj[winnerPlayer]);
                }
        ...
        return () => clearTimeout(tmInterval);
    });
    ...
    */
    useEffect(() => {
        window.onbeforeunload = (e) => {
            history.push('/single-player', usersObj);
        };

        gridInstance = new GridClass(rows, columns);
        gridInstance.initialiseState();
        gridInstance.addUserNames(usersObj['user1'].userName, 'Machine');
